...
echo "Creating configuration at: /etc/mysql/mariadb.conf.d/50-mariadb-clients.cnf"
cat <<EOF | sudo tee "/etc/mysql/mariadb.conf.d/50-mariadb-clients.cnf" > /dev/null
[mariadb-client]
...
echo "Removing undo files: /var/lib/mysql/"
sudo find /var/lib/mysql/ -name undo* -exec sh -c "echo \"Removing file: {}\"; rm -rf {}" \;
