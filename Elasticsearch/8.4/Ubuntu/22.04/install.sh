...
Example: ${scriptName} --applicationName Elasticsearch --applicationVersion 8.4 --bindAddress 0.0.0.0 --port 9200
EOF
}
...
add-asc-repository "elastic-8.x.list" "https://artifacts.elastic.co/packages/8.x/apt" "stable" "main" "https://artifacts.elastic.co/GPG-KEY-elasticsearch" "n"

echo "Installing Elasticsearch 8.4"
install-package elasticsearch 8.4
...
replace-file-content /etc/elasticsearch/elasticsearch.yml "xpack.security.enabled: false" "xpack.security.enabled: true"
...
stop() {
  ...
  cat /var/run/elasticsearch/elasticsearch.pid | xargs kill -15
  exit
}
for command in "\$@"; do
  echo "Run: \${command}"
