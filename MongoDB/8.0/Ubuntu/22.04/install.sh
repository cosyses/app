...
EOF
  ...
  if [[ -d /usr/local/lib/start/ ]]; then
    echo "Creating start script at: /usr/local/lib/start/10-mongodb.sh"
    cat <<EOF > /usr/local/lib/start/10-mongodb.sh
#!/usr/bin/env bash
echo "Starting MongoDB"
mkdir -p /var/run/mongod/
chown mongodb: /var/run/mongod/
rm -rf /usr/local/lib/start/container.out
touch /usr/local/lib/start/container.out
chown mongodb: /usr/local/lib/start/container.out
sudo -H -u mongodb bash -c "nohup /usr/bin/mongod -f /etc/mongod.conf --pidfilepath /var/run/mongod/mongodb.pid > /usr/local/lib/start/container.out 2>&1 &" &
EOF
    chmod +x /usr/local/lib/start/10-mongodb.sh
  fi

  if [[ -d /usr/local/lib/stop/ ]]; then
    echo "Creating stop script at: /usr/local/lib/stop/10-mongodb.sh"
    cat <<EOF > /usr/local/lib/stop/10-mongodb.sh
#!/usr/bin/env bash
echo "Stopping MongoDB"
sudo -H -u mongodb bash -c "/usr/bin/mongod -f /etc/mongod.conf --shutdown"
EOF
    chmod +x /usr/local/lib/stop/10-mongodb.sh
  fi
fi
