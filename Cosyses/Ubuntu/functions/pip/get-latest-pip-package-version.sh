...
versions=($(get-available-pip-package-versions "${packageName}" "${baseVersion}"))

if [[ "${#versions}" -eq 0 ]]; then
  >&2 echo "Could not find latest version for pip package: ${packageName} in version: ${baseVersion}"
  exit 1
fi
