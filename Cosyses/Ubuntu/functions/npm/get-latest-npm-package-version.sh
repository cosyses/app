...
versions=($(get-available-npm-package-versions "${packageName}" "${baseVersion}"))

if [[ "${#versions}" -eq 0 ]]; then
  >&2 echo "Could not find latest version for npm package: ${packageName} in version: ${baseVersion}"
  exit 1
fi
